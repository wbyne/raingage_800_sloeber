...
boolean Adafruit_FONA::writeSMS(char *smsaddr, char *smsmsg)
{
	if (!sendCheckReply("AT+CMGF=1", "OK")) 
		return -1;
	
	char sendcmdw[30] = "AT+CMGW=\"";
	strncpy(sendcmdw+9, smsaddr, 30-9-2);  // 9 bytes beginning, 2 bytes for close quote + null
	sendcmdw[strlen(sendcmdw)] = '\"';
	 if (! sendCheckReply(sendcmdw, "> ")) return false;
#ifdef ADAFRUIT_FONA_DEBUG
  Serial.print("> "); Serial.println(smsmsg);
#endif
  mySerial->println(smsmsg);
...
return sendCheckReply(sendbuff, "OK", 5000);
